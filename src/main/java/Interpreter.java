...
public class Interpreter {
    ...
    private void magic(ExpressionOperation operation) {
        ...
        if (isStrings(operation)){
           if ( operation.getOperator().value.equals("+")){
        ...
        if (operation.getOperator().isArithmetical()) {
    ...
    private boolean isStrings(ExpressionOperation operation) {
        return operation.getFirst().variableType.equals
                (operation.getSecond().variableType) && operation.getFirst().variableType.equals(VariableType.string);
    }
    ...
    private void updateOperationVariablesValues(final ExpressionOperation operation) {
        ...
        if (second.value.startsWith("\"") || second.value.endsWith("\"")) {
            second.variableType = VariableType.string;
            return;
    ...
    private String executeString(String first, String second, String operator) {
        switch (operator) {
            case "+":
                return first + second;
        }
        try {
            throw new Exception("Error ExecuteString " + first + " " + second + " " + operator);
        } catch (Exception e) {
            System.out.println(e.getMessage());
        }
        return null;
